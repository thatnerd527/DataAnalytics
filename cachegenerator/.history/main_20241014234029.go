...
func processRange(wrange WorkerRange, startdate time.Time, comc chan<- WorkerResult) {
	...
	daydict := make(map[string][]Record)
...
func main() {
	...
	alltimeclose := make([]float64, int(daysbetween))
	alltimeopen := make([]float64, int(daysbetween))
	alltimevolume := make([]float64, int(daysbetween))
	periods := make([]time.Time, int(daysbetween))
