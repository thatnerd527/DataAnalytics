...
func uniDate(date time.Time) string {
	return fmt.Sprintf("%d-%d-%d", date.Year(), date.Month(), date.Day())
}

func processRange(wrange WorkerRange, startdate time.Time, comc chan<- WorkerResult) {
	amount := wrange.Max - wrange.Min
	day1 := startdate.AddDate(0, 0, int(wrange.Min))
	close := make([]float64, amount)
	open := make([]float64, amount)
	volume := make([]float64, amount)
	periods := make([]time.Time, amount)
	fmt.Println("Processing data for range ", wrange.Min, " to ", wrange.Max)
	timetmp := map[string]bool{}
	for i := 0; i < int(amount); i++ {
		day := day1.AddDate(0, 0, int(i))
		timetmp[uniDate(day)] = true
	}
	daydict := map[string][]Record{}

	for _, record := range globaldata {
		if timetmp[uniDate(record.Date)] {
			if daydict[uniDate(record.Date)] == nil {
				daydict[uniDate(record.Date)] = make([]Record, 0)
			}
			daydict[uniDate(record.Date)] = append(daydict[uniDate(record.Date)], record)
		}
	}
	for i := 0; i < int(amount); i++ {
		day := day1.AddDate(0, 0, int(i))
		periods[i] = day
		fetch := daydict[uniDate(day)]
		if len(fetch) == 0 {
			close[i] = 0
			open[i] = 0
			volume[i] = 0
	...
	fmt.Println("Data for range ", wrange.Min, " to ", wrange.Max, " processed")
	comc <- WorkerResult{
		Year: 0,
		Period: periods,
		Close: close,
...
func main() {
	// Part 1
	// read the csv file
	inputfile := flag.String("input", "data.csv", "input file")
	outputpostfix := flag.String("outputsuffix", "", "output file suffix")
	flag.Parse()
	...
	os.WriteFile(fmt.Sprintf("peryearclose%v.json",*outputpostfix), marshalled, 0644)
	marshalled, err = json.Marshal(peryearopen)
	if (err != nil) {
		panic(err)
	}
	os.WriteFile(fmt.Sprintf("peryearopen%v.json",*outputpostfix), marshalled, 0644)
	marshalled, err = json.Marshal(peryearvolume)
	if (err != nil) {
		panic(err)
	}
	os.WriteFile(fmt.Sprintf("peryearvolume%v.json",*outputpostfix), marshalled, 0644)

	// 2nd part
	fmt.Println("Processing data for periods")
	...
	resultsdata = make([]WorkerResult, len(ranges))
	for _, wrange := range ranges {
		go processRange(wrange, lowestperiod, results)
	}
	for i := 0; i < len(ranges); i++ {
		resultsdata[i] = <-results
	}

	alltimeclose := make([]float64, 0)
	alltimeopen := make([]float64,0)
	alltimevolume := make([]float64, 0)
	periods := make([]string, 0)
	for _, results := range(resultsdata) {
		alltimeclose = append(alltimeclose, results.Close...)
		alltimeopen = append(alltimeopen, results.Open...)
		alltimevolume = append(alltimevolume, results.Volume...)
		periods = append(periods, pie.Map(results.Period, func(t time.Time) string {
			return fmt.Sprintf("%d-%d-%d", t.Year(), t.Month(), t.Day())
		})...)
	}
	fmt.Println("Processing data for periods done")

	marshalled, err = json.Marshal(alltimeclose)
	if (err != nil) {
		panic(err)
	}
	os.WriteFile(fmt.Sprintf("alltimeclose%v.json",*outputpostfix), marshalled, 0644)
	marshalled, err = json.Marshal(alltimeopen)
	if (err != nil) {
		panic(err)
	}
	os.WriteFile(fmt.Sprintf("alltimeopen%v.json",*outputpostfix), marshalled, 0644)
	marshalled, err = json.Marshal(alltimevolume)
	if (err != nil) {
		panic(err)
	}
	os.WriteFile(fmt.Sprintf("alltimevolume%v.json",*outputpostfix), marshalled, 0644)
	marshalled, err = json.Marshal(periods)
	if (err != nil) {
		panic(err)
	}
	os.WriteFile(fmt.Sprintf("periods%v.json",*outputpostfix), marshalled, 0644)
