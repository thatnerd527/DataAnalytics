...
func processRange(wrange WorkerRange, startdate time.Time, comc chan<- WorkerResult) {
	...
	timetmp := map[int64]bool{}
	for i := 0; i < int(amount); i++ {
		day := day1.AddDate(0, 0, int(i))
		timetmp[day.UnixNano()] = true
	}
	daydict := make(map[int64][]Record)

	for _, record := range globaldata {
		if timetmp[record.Date.UnixNano()] {
			daydict[record.Date.UnixNano()] = append(daydict[record.Date.UnixNano()], record)
		}
	}
	for i := 0; i < int(amount); i++ {
		day := day1.AddDate(0, 0, int(i))
		periods[i] = day
		fetch := daydict[day.UnixNano()]
		if len(fetch) == 0 {
			close[i] = 0
			open[i] = 0
			volume[i] = 0
